...
- **TensorFlow is built with bazel**: the first build runs a full TensorFlow bazel build (thousands of actions, so it takes a long time). Interrupting it is safe because completed bazel actions are cached under `~/bazel_root`, so a rerun resumes roughly where it stopped. The bazel build is shared between variants; only the library install, the `tensorflow_runtime.o` compile and the link are per-variant.
