...
- `docs/parrots/Executable Shape.md` - the linking model: locally built static archives named explicitly, the five deliberate shared library dependencies, the fixed load address on both platforms and its copy-relocation cost on ELF, the empty dynamic symbol table, zstd-compressed debug info embedded in fast and debug, the PDB written for every Windows variant, the split-DWARF and fully stripped Linux release
- `docs/parrots/Error Recovery.md` - the Error model from error.hpp (one Error per Object, Errors pause their Object, reporters, clearing, what is not implemented yet), the AutomatLoop catch that turns a LowLevelError into a reported Error, and the low-level layer: the Linux signal handlers and the personality wrapper that recovers the faulting frame's cleanups from its exception table, the Windows vectored exception handler that redirects the faulting thread into a function that throws, per-thread SignalStacks (alternate signal stack on Linux, stack guarantee and recovery registration on Windows), stack overflow guard restoration, lazy source location symbolization
