...
- **Board** (`src/board.hpp`) - Container for Locations; several boards can own the same Object. The board list is `engine.boards` (`src/engine.hpp`); `engine.mutex` guards it together with the order of every board's `locations`. Ownership itself is recorded by `Owned<T>` links (`src/object.hpp`), which let an Object enumerate its owners without a scan (see `docs/parrots/Object Ownership.md`)
- **ToyScope** (`src/toy.hpp`) - Maps `(owner, atom)` keys to Toy widgets. Each `BoardWidget` owns the scope for its board (this enforces one widget per object per board); `RootWidget` keeps the scope for top-level widgets; `Widget::ToyScope()` resolves to the nearest enclosing scope
...
Objects (multi-threaded) notify Toys (UI-thread) via `monitor`:
- Object bumps `monitor.fetch_add(1, relaxed)` on state change
- `{Toy,Action}::{On,}Poll` compares toy's counter with `observed_monitor` and calls `WakeAnimationAt()`
...
- `docs/parrots/Objects and Modalities.md` - the layering model: Objects are modality-independent cores; the 2D board UI is one modality among possible others (text, 3D, RPC); which data may live Engine-side (O(1) per-type hints, Board/Location placement), placement requests instead of Engine-side coordinates, interface-to-widget mapping, and monitors as the modality-agnostic notification channel
- `docs/parrots/Boards.md` - multi-board ownership: one Object owned by several Boards with one resident widget each, per-board ToyScopes, the three drag modes with entry-time ownership transfer, the merge drop onto a board that already owns the object, dropping on the background to create a new board, and per-board connection drawing
...
- `docs/parrots/Executable Shape.md` - the linking model: locally built static archives named explicitly, the five deliberate shared library dependencies, the fixed load address on both platforms and its copy-relocation cost on ELF, the empty dynamic symbol table, zstd-compressed debug info embedded in every Linux variant, the release symbol tables stripped after the link, the PDB written for every Windows variant
- `docs/parrots/Error Recovery.md` - the Error model from error.hpp (one Error per Object, Errors pause their Object, reporters, clearing, what is not implemented yet), the AutomatLoop catch that turns a LowLevelError into a reported Error, and the low-level layer: fault interception (the Linux signal handler that throws, the Windows vectored exception handler that redirects the faulting thread into a function that throws), the shared next-call-site rule that re-aims the faulting instruction pointer so the faulting frame's destructors run, the Linux personality wrapper that skips uncovered frames instead of terminating, per-thread SignalStacks (alternate signal stack on Linux, stack guarantee and recovery registration on Windows), stack overflow guard restoration, lazy source location symbolization
...
- `docs/parrots/Object Ownership.md` - structural ownership through `Owned<T>` links: the intrusive owner list inside every Object, `Object::Owners()`, the home as the first owner (`HomeLocation`, `Make Home`, the home recorded in the saved state and restored by rotating the owner list), `LocationOrNull` without board scans, which holders are links and which stay `Ptr`, the one-byte per-object spinlock and why it was chosen over `engine.mutex`, the single-lock protocol and how a dying owner is skipped
- `docs/parrots/Options.md` - every Interface is an option: activation through `Interface::Table` (`activate`, `make_icon`, `cursor`) and the Toy passed to it, commands as Command interfaces, the Scalar, Text and ObjectSource interfaces, the widget side (`FindOption` over buttons, keys and menu directions, `MenuMode`, the owner lock held by the walkers), action zones, the fixed-slot bubble menu, the chained menus with S reserved for the parent and the rationale against directional fallback, the rules for `FindOption`
...
- **Interface references**: a field stores a `Linked<T>` (`src/interface.hpp`), which holds no reference; `Lock()` turns it into a `Locked<T>`, a binding that keeps its object alive and is used like the interface itself. `Unsafe()` reads the binding without locking, for callers that already hold the object.
- **Casting**: a wrapper (`Ptr<T>`, a `Locked<T>` or an interface binding) is cast with the LLVM spellings from `src/casting.hpp`: `dyn_cast` returns null when the type does not match, `cast` asserts it in debug builds, and the `*_if_present` variants accept a null input. A raw pointer uses plain `dynamic_cast` or `static_cast`, because the LLVM machinery needs a `classof` that the Object and Widget hierarchies do not have. The smart pointers carry no cast members of their own: casting a `Ptr` yields a `Ptr` that shares the object, and a weak pointer is locked first and cast afterwards.
...
- **Texture anchors are a Widget member, not a virtual**: `Widget::texture_anchors` must be kept up to date by the widget's `Tick` (see `SyncBelt::Tick`, the `UpdateTextureAnchors` calls in ui_connection_widget.cpp, and `LocationWidget::AnchorToPointer`). The renderer snapshots the member at pack time and publishes fresh values each frame; the `WARP` compositor warps stale textures so each anchor lands at its current position, blending every anchor's pull against `Widget::local_to_parent_weight`, and a pointer-bound anchor (`TextureAnchor::pointer`, a MortalPtr) additionally follows that pointer's movement since the frame was packed, read from globals sampled just before compositing starts.
- **Culling includes texture-anchor displacements**: The `WARP` compositor can draw a texture far away from the widget's own bounds, so visibility cannot be decided from those bounds alone. During frame packing, every tree node stores the displacements of its own anchors together with the displacements of all ancestor anchors. The displacements are expressed in root pixels, and a root-pixel displacement applies to the whole subtree, because contents baked into a warped texture are displaced together with it. Visibility is tested with the widget bounds expanded by these displacements (`ExpandByDisplacements`), and large textures are clipped to the root widget bounds expanded by the negated displacements, so the content that may be warped on-screen is still rasterized. The bounds returned by `WidgetDrawable::onGetBounds` are expanded to include `warp.draw_bounds`, because they become the cull rect of the picture snapshot that composites the texture, and `SkCanvas::drawPicture` skips pictures whose cull rect lies outside the canvas clip.
...
- **`Syncable::Table::DefaultFind` returns a `Locked<Interface>` with a null table**: Because Gears are top-level Objects, not interfaces. `Interface::operator bool()` requires both the object and the table, so `(bool)arg.Find()` returns false even when connected to a Gear. Use `Argument::IsConnected()` instead, which only tests the object.
- **ToyScope is UI-thread only**: Not synchronized. Engine threads must not call `FindOrMake` or modify ToyScope. Signal via `monitor` instead.
- **Children of a clipped widget are not ticked**: the renderer skips the subtree of any widget whose `DrawBounds` land off-screen (renderer.cpp step 2), even when a child (such as the autoconnect radar) would reach back on-screen. A widget whose `DrawBounds` return `nullopt` has no texture of its own, but its children are still visited and ticked. Returning an empty rect from `DrawBounds` is invalid; non-release builds crash on it so the widget can be fixed to return `nullopt`.
- **`RootWidget::Poll` runs before widget tree is built**: It polls the root scope, which dispatches into every board's scope via the virtual `BoardWidget::Poll`, and in-progress actions (boards sleep when idle, so their scopes can't be polled from `BoardWidget::Tick`). Dead entries are cleaned here, but widgets that call `MarkDead` during the tree traversal (after `RootWidget::Poll`) won't be cleaned until the next frame.
...
- **`RootWidget::OnChildDead` removes from `children`**: Dead children are removed immediately when `MarkDead` fires (during the child's Tick). The old `std::erase_if(children, dead)` sweep was removed because `ToyScope::Poll` may destroy widgets before the sweep runs, creating dangling pointers in `children`.
- **`Widget::FindOption` runs on every pointer move**: `Pointer::UpdatePath` calls it (through `CheckCursorChanged`) for the hovered widget and its ancestors, and `RootWidget::Tick` calls `UpdatePath` on every animated frame. `FindOption()` must only describe the current state: no mutation, no object mutexes beyond the owner lock the walker already holds, no widget creation, no `WakeAnimation`, no pointer position (parts with their own gesture are `ui::ActionZone` children). Everything that changes state belongs in the interface's `activate`.
