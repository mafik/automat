...
- `docs/parrots/Object Ownership.md` - structural ownership through `Owned<T>` links: the intrusive owner list inside every Object, `Object::Owners()`, `MyLocation` and `LocationOrNull` without board scans, which holders are links and which stay `Ptr`, the one-byte per-object spinlock and why it was chosen over `engine.mutex`, the single-lock protocol and how a dying owner is skipped
...
- **Interface references**: a field stores a `Stored<T>` (`src/interface.hpp`), which holds no reference; `Lock()` turns it into a `Locked<T>`, a binding that keeps its object alive and is used like the interface itself. `Unsafe()` reads the binding without locking, for callers that already hold the object.
