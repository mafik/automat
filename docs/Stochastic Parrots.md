...
- **Board** (`src/board.hpp`) - Container for Locations; several boards can own the same Object. The board list is `engine.boards` (`src/engine.hpp`); `engine.mutex` guards it together with every board's `locations`
