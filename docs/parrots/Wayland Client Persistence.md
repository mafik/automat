...
so a loaded board immediately looks the way it was saved.
...
re-running). The fit with Automat's principle of exposing low-level
primitives is good; the engineering cost is the reason it is not built.
...
This is the only option where cloning a running process is well-defined —
...
them) and "restore" by replaying. This works for stateless demo clients and
fails for everything real: replay re-executes outputs, not inputs — the
client's decisions depended on timing, file contents and randomness that
replay does not reproduce, and the client process itself still has to exist
to continue past the recording. Replay is the right tool for testing
...
startup cost but diverges from an already-initialized state. Wayland makes
the naive version illegal — a forked client would share one socket with its
...
2. **CRIU for cooperative processes** (option 3) — large, real value for
   terminals and editors, needs a proxy or reconnect mechanism for the socket.
...
arrives later can extend it without a format migration.
