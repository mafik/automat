...
Accessor::Accessor(UsagePage usage_page, Usage usage, uint32_t bit_offset,
                   uint32_t bit_width, uint32_t hid_logical_minimum,
                   uint32_t hid_logical_maximum, uint32_t hid_physical_minimum,
                   uint32_t hid_physical_maximum, int8_t hid_exponent,
                   Unit unit)
    : usage_page(usage_page), usage(usage), bit_offset(bit_offset),
      bit_width(bit_width), logical_minimum(hid_logical_minimum),
      logical_maximum(hid_logical_maximum) {
  if ((hid_physical_maximum == UINT32_MAX) ||
      (hid_physical_minimum == UINT32_MAX) ||
...
uint32_t Accessor::Read<uint32_t>(const uint8_t *report,
  ...
  if (bit_offset + bit_width > report_bytes * 8)
    return 0;
...
template <>
double Accessor::Read<double>(const uint8_t *report,
                              size_t report_bytes) const {
  uint32_t raw_value = Read<uint32_t>(report, report_bytes);
  if (raw_value < logical_minimum)
    return physical_minimum;
  if (raw_value > logical_maximum)
    return physical_maximum;
  return physical_minimum + (physical_maximum - physical_minimum) *
                                (raw_value - logical_minimum) /
...
void ParseReportDescriptor(
  ...
  while (pos < report_descriptor_bytes) {
    ...
    ItemCategory category =
        (ItemCategory)((report_descriptor[pos] >> 2) & 0b11);
    ...
    if (bTag == 0b1111 && category == kItemCategoryReserved && bSize == 0b10) {
      ERROR()
          << "Encountered a \"long tag\" when parsing HID report descriptor. "
          ...
          if (opts.encoding == Encoding::Array) {
            ...
            for (int i = 0; i < report_count; ++i) {
              ...
              hid::Accessor accessor = hid::Accessor(
                  usage_page, usage, bit_offset, report_size, logical_minimum,
                  logical_maximum, physical_minimum, physical_maximum, exponent,
                  unit);
