...
/** @brief Get the container ID for a HID device.
        ...
        Since version 0.12.0, @ref HID_API_VERSION >= HID_API_MAKE_VERSION(0,
   12, 0)
 ...
 * structure. This reconstructed report descriptor is logical identical to the
 * real report descriptor, but not byte wise identical.
 *
 * @param[in]  hidp_preparsed_data Pointer to the HIDP_PREPARSED_DATA to read,
 * i.e.: the value of PHIDP_PREPARSED_DATA, as returned by HidD_GetPreparsedData
 * WinAPI function.
 * @param      buf       Pointer to the buffer where the report descriptor
 * should be stored.
 * @param[in]  buf_size  Size of the buffer. The recommended size for the buffer
 * is @ref HID_API_MAX_REPORT_DESCRIPTOR_SIZE bytes.
 *
