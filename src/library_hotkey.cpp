...
bool HotKey::DeserializeKey(ObjectDeserializer& d, StrView keyName) {
  ...
  auto ModifySetting = [&](auto&& setter) {
    if (was_on) enabled->TurnOff();
    setter();
    if (was_on) enabled->TurnOn();
  };
...
struct HotKeyWidget : ObjectToy {
  ...
  HotKeyWidget(ui::Widget* parent, Object& hotkey_obj) : ObjectToy(parent, hotkey_obj) {
    ...
    ctrl_button->target = Linked<>(weak, &HotKey::toggle_ctrl_tbl);
    alt_button->target = Linked<>(weak, &HotKey::toggle_alt_tbl);
    shift_button->target = Linked<>(weak, &HotKey::toggle_shift_tbl);
    windows_button->target = Linked<>(weak, &HotKey::toggle_super_tbl);
    shortcut_button->target = Linked<>(weak, &kSelectHotKey);
  }
  ...
  void Draw(SkCanvas& canvas) const override {
    ...
    SkColor4f border_colors[] = {"#f0f0f0"_color4f, "#cccccc"_color4f};
    ...
    SkColor4f light_colors[] = {"#fdf8e0"_color4f, "#111c22"_color4f};
    SkPaint light_paint;
    ...
    SkColor4f shadow_colors[] = {"#111c22"_color4f, "#fdf8e0"_color4f};
