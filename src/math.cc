...
std::string Vec2::ToStr() const { return f("Vec2({}, {})", x, y); }
std::string Vec3::ToStr() const { return f("Vec3({}, {}, {})", x, y, z); }
std::string Rect::ToStr() const {
  return f("Rect(t={}, r={}, b={}, l={})", top, right, bottom, left);
}

std::string ToStrMetric(float x) { return f("{:4.1f}cm", x * 100); }
...
std::string Vec2::ToStrPx() const { return f("{:.0f}x{:.0f}px", roundf(x), roundf(y)); }
