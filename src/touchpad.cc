...
std::string UTF16ToUTF8(const wchar_t *utf16) {
  ...
  int utf8_len =
      SkUTF::UTF16ToUTF8(utf8_buffer, utf8_cap, (uint16_t *)utf16, utf16_len);
...
struct ReportAccessor {
  uint8_t report_id = 0;
  std::optional<hid::Accessor> touch_valid, tip_switch, button1,
      contact_identifier, contact_count, x, y, scan_time;
  void ProcessInput(uint8_t *report, size_t report_bytes) {
    ...
    if (contact_count) {
      log_message +=
          " contact_count=" +
          f("%d", contact_count->Read<uint32_t>(report, report_bytes));
    }
    if (x) {
    ...
    if (scan_time) {
      log_message += " scan_time=" +
                     f("%f", scan_time->Read<double>(report, report_bytes));
    }
    if (button1) {
      log_message +=
          " button1=" + f("%d", button1->Read<bool>(report, report_bytes));
    }
    LOG() << log_message;
  }
};
...
bool ShouldIgnoreScrollEvents() {
  return time::now() < last_pan_time + time::duration(1.0);
}
...
struct TouchPadImpl {
  ...
  TouchPadImpl(std::string_view path) : path(path) {
    touchpads.push_back(&touchpad);
  }
  // Can't be copied because `touchpad` is referenced from `touchpads`.
  TouchPadImpl(const TouchPadImpl &) = delete;
  ~TouchPadImpl() {
    touchpads.erase(std::find(touchpads.begin(), touchpads.end(), &touchpad));
  }
  ...
  void Init() {
    ...
    int report_descriptor_size = hid_get_report_descriptor(
        dev, report_descriptor, HID_API_MAX_REPORT_DESCRIPTOR_SIZE);
    hid_close(dev);
    ...
    hid::ParseReportDescriptor(
        ...
        [&](uint8_t report_id, hid::Accessor &accessor) {
          ReportAccessor &report_processor =
              GetOrCreateReportProcessor(report_id);
          ...
          if (accessor.usage_page == hid::UsagePage_Digitizer) {
            ...
            } else if (accessor.usage ==
                       hid::Usage_Digitizer_ContactIdentifier) {
            ...
            if (accessor.usage == hid::Usage_GenericDesktop_X) {
              ...
              touchpad.width_m =
                  (accessor.physical_maximum - accessor.physical_minimum);
          ...
          if (target_field) {
            ...
            LOG() << "Unknown HID input. Usage Page: "
                  << UsagePageToString(accessor.usage_page)
                  << f(" (0x%04X)", accessor.usage_page) << " Usage: "
  ...
  void ProcessInputReport(uint8_t *data, size_t len) {
    ...
    for (auto &report_accessor : report_accessors) {
      ...
      if (report_accessor.button1) {
        touchpad.buttons[0] =
            report_accessor.button1->Read<bool>(report, report_bytes);
      }
      if (report_accessor.contact_count) {
        ...
        uint32_t cc =
            report_accessor.contact_count->Read<uint32_t>(report, report_bytes);
        if (cc) {
          contact_count = cc;
          contact_i = 0;
        }
      ...
      if (report_accessor.contact_identifier) {
        uint32_t touch_id = report_accessor.contact_identifier->Read<uint32_t>(
            report, report_bytes);
        ...
        if (report_accessor.touch_valid) {
          touch_valid =
              report_accessor.touch_valid->Read<bool>(report, report_bytes);
        }
        if (touch_valid) {
          bool tip_switch = true;
          if (report_accessor.tip_switch) {
            tip_switch =
                report_accessor.tip_switch->Read<bool>(report, report_bytes);
          }
          if (tip_switch) {
...
void Init() {
  ...
  RAWINPUTDEVICE rid = {.usUsagePage = hid::UsagePage_Digitizer,
                        ...
                        .dwFlags =
                            RIDEV_DEVNOTIFY, // Request WM_INPUT_DEVICE_CHANGE
...
std::string GetRawInputDeviceName(HANDLE device) {
  ...
  UINT ret =
      GetRawInputDeviceInfoW(device, RIDI_DEVICENAME, NULL, &device_name_chars);
  if (ret) {
    ERROR() << "Error when retrieving device name size. Error code: "
            << GetLastError();
    return "";
  }
  wchar_t device_name_utf16[device_name_chars];
  UINT device_name_copied = GetRawInputDeviceInfoW(
      device, RIDI_DEVICENAME, device_name_utf16, &device_name_chars);
  if (device_name_chars != device_name_copied) {
    ERROR() << "Error when retrieving device name. Requested size="
            << device_name_chars << ", Copied size=" << device_name_copied
...
std::optional<LRESULT> ProcessEvent(UINT msg, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_INPUT: {
    ...
    UINT size = 0;
    UINT ret = GetRawInputData(hRawInput, RID_INPUT, NULL, &size,
                               sizeof(RAWINPUTHEADER));
    if (ret == (UINT)-1) { // error when retrieving size of buffer
      ERROR() << "Error when retrieving size of buffer. Error code: "
              << GetLastError();
    ...
    UINT size_copied = GetRawInputData(hRawInput, RID_INPUT, raw_input_buffer,
    ...
    if (raw_input->header.dwType != RIM_TYPEHID) {
      ERROR() << "Unexpected RAWINPUTHEADER.dwType: "
    ...
    if (!touchpad) {
      std::string device_name =
          GetRawInputDeviceName(raw_input->header.hDevice);
      for (TouchPadImpl &impl : touchpad_impls) {
