...
bool LevelGrabsMarker(const Rect& band, Vec2 p, float value, float vmin, float vmax,
                      float grip = 0.f);
...
Rect ConnectivityCell(const Rect& r, int which);  // which: 0 = 4-conn, 1 = 8-conn
int ConnectivityHit(const Rect& r, Vec2 p);       // 0 = miss, else 4 or 8
...
// selection is dimmed.
int RegionHit(const Rect& mq, Vec2 p,
...
bool TransformRingHit(Vec2 c, float radius, Vec2 p, float band = 1.75_mm);
float TransformRingAngleAt(Vec2 c, Vec2 p);  // degrees, 0 = up, clockwise
...
int PaletteHit(const Rect& r, Vec2 p);  // -1 = miss, else the palette index
...
int ModeWheelHit(Vec2 c, float radius, Vec2 p, int n);  // nearest option, or -1 outside
...
int ChannelTapAt(const Rect& r, Vec2 p, int n);  // -1 = miss, else the tap index
void DrawChannelTap(SkCanvas& canvas, const Rect& r, int selected, beta::State state, uint32_t seed,
                    const char* const* labels = nullptr, const SkColor* colors = nullptr,
                    int n = 4);
