...
struct Subsurface : Common {
  ...
  MortalPtr<Surface> surface;  // the child surface (holds the subsurface role)
...
struct XdgToplevel : Common {
  ...
  I64 pid = 0;
  WeakPtr<library::WaylandWindow> window;
...
struct Surface : Common {
  ...
  MortalPtr<XdgSurface> xdg;
  MortalPtr<Subsurface> as_subsurface;
  MortalPtr<Viewport> viewport;
  MortalPtr<Buffer> held_dmabuf;  // dmabuf wl_buffer held for zero-copy display
  SurfaceCutout content;
  // The board object mirroring this surface (a Ptr because the object holds a MortalPtr back here).
  Ptr<library::WaylandSurface> object;
