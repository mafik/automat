...
static struct hid_api_version api_version = {
    .major = HID_API_VERSION_MAJOR, .minor = HID_API_VERSION_MINOR, .patch = HID_API_VERSION_PATCH};
...
static void register_winapi_error_to_buffer(wchar_t** error_buffer, const WCHAR* op) {
  ...
  *error_buffer = NULL;
  ...
  DWORD system_err_len = FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
                                        ...
                                        system_err_buf, ARRAYSIZE(system_err_buf), NULL);
  ...
  *error_buffer = (WCHAR*)calloc(msg_len + 1, sizeof(WCHAR));
  ...
  int printf_written = swprintf(msg, msg_len + 1, L"%.*ls: (0x%08X) %.*ls", op_len, op, error_code,
                                system_err_len, system_err_buf);
...
static HANDLE open_device(const wchar_t* path, BOOL open_rw) {
  ...
  handle = CreateFileW(path, desired_access, share_mode, NULL, OPEN_EXISTING,
...
HID_API_EXPORT const struct hid_api_version* HID_API_CALL hid_version() { return &api_version; }

HID_API_EXPORT const char* HID_API_CALL hid_version_str() { return HID_API_VERSION_STR; }
...
static void* hid_internal_get_devnode_property(DEVINST dev_node, const DEVPROPKEY* property_key,
...
static void* hid_internal_get_device_interface_property(const wchar_t* interface_path,
  ...
  cr = CM_Get_Device_Interface_PropertyW(interface_path, property_key, &property_type, NULL, &len,
                                         0);
  if (cr != CR_BUFFER_SMALL || property_type != expected_property_type) return NULL;

  property_value = (PBYTE)calloc(len, sizeof(BYTE));
  cr = CM_Get_Device_Interface_PropertyW(interface_path, property_key, &property_type,
...
static void hid_internal_get_usb_info(struct hid_device_info* dev, DEVINST dev_node) {
  ...
  device_id =
      hid_internal_get_devnode_property(dev_node, &DEVPKEY_Device_InstanceId, DEVPROP_TYPE_STRING);
  ...
  */
  ...
  hardware_ids = hid_internal_get_devnode_property(dev_node, &DEVPKEY_Device_HardwareIds,
  ...
  */
  ...
  if (wcslen(dev->manufacturer_string) == 0) {
    wchar_t* manufacturer_string = hid_internal_get_devnode_property(
        dev_node, &DEVPKEY_Device_Manufacturer, DEVPROP_TYPE_STRING);
  ...
  if (wcslen(dev->serial_number) == 0) {
    ...
    free(device_id);
    device_id = hid_internal_get_devnode_property(usb_dev_node, &DEVPKEY_Device_InstanceId,
                                                  DEVPROP_TYPE_STRING);
    if (!device_id) goto end;
...
/* HidD_GetProductString/HidD_GetManufacturerString/HidD_GetSerialNumberString is not working for
   BLE HID devices Request this info via dev node properties instead.
...
static void hid_internal_get_ble_info(struct hid_device_info* dev, DEVINST dev_node) {
  ...
  if (wcslen(dev->serial_number) == 0) {
    ...
    wchar_t* serial_number = hid_internal_get_devnode_property(
  ...
  if (wcslen(dev->product_string) == 0) {
    ...
    wchar_t* product_string = hid_internal_get_devnode_property(
    ...
    if (!product_string) {
      ...
      if (CM_Get_Parent(&parent_dev_node, dev_node, 0) == CR_SUCCESS) {
        ...
        product_string =
            hid_internal_get_devnode_property(parent_dev_node, &DEVPKEY_NAME, DEVPROP_TYPE_STRING);
      }
    }
...
static void hid_internal_get_info(const wchar_t* interface_path, struct hid_device_info* dev) {
  ...
  device_id = hid_internal_get_device_interface_property(interface_path, &DEVPKEY_Device_InstanceId,
                                                         DEVPROP_TYPE_STRING);
  if (!device_id) goto end;
  ...
  compatible_ids = hid_internal_get_devnode_property(dev_node, &DEVPKEY_Device_CompatibleIds,
                                                     DEVPROP_TYPE_STRING_LIST);
  if (!compatible_ids) goto end;

  /* Now we can parse parent's compatible IDs to find out the device bus type */
  for (wchar_t* compatible_id = compatible_ids; *compatible_id;
       compatible_id += wcslen(compatible_id) + 1) {
    /* Normalize to upper case */
...
struct hid_device_info HID_API_EXPORT* HID_API_CALL hid_enumerate(unsigned short vendor_id,
  ...
  do {
    cr = CM_Get_Device_Interface_List_SizeW(&len, &interface_class_guid, NULL,
    ...
    cr = CM_Get_Device_Interface_ListW(&interface_class_guid, NULL, device_interface_list, len,
                                       CM_GET_DEVICE_INTERFACE_LIST_PRESENT);
...
HID_API_EXPORT hid_device* HID_API_CALL hid_open(unsigned short vendor_id,
  ...
  /* TODO: Merge this functions with the Linux version. This function should be platform
   * independent. */
...
int HID_API_EXPORT HID_API_CALL hid_write(hid_device* dev, const unsigned char* data,
                                          size_t length) {
  ...
  if (length >= dev->output_report_length) {
    ...
    if (dev->write_buf == NULL) dev->write_buf = (unsigned char*)malloc(dev->output_report_length);
    buf = dev->write_buf;
...
int HID_API_EXPORT HID_API_CALL hid_read_timeout(hid_device* dev, unsigned char* data,
  ...
  if (!dev->read_pending) {
    ...
    ResetEvent(ev);
    res = ReadFile(dev->device_handle, dev->read_buf, (DWORD)dev->input_report_length, &bytes_read,
                   &dev->ol);
...
int HID_API_EXPORT HID_API_CALL hid_send_feature_report(hid_device* dev, const unsigned char* data,
                                                        size_t length) {
  BOOL res = FALSE;
  unsigned char* buf;
...
static int hid_get_report(hid_device* dev, DWORD report_type, unsigned char* data, size_t length) {
  ...
  res = DeviceIoControl(dev->device_handle, report_type, data, (DWORD)length, data, (DWORD)length,
                        &bytes_returned, &ol);
...
int HID_API_EXPORT HID_API_CALL hid_get_feature_report(hid_device* dev, unsigned char* data,
                                                       size_t length) {
...
int HID_API_EXPORT HID_API_CALL hid_get_input_report(hid_device* dev, unsigned char* data,
                                                     size_t length) {
  /* We could use HidD_GetInputReport() instead, but it doesn't give us an actual length,
   * unfortunately */
...
int HID_API_EXPORT_CALL HID_API_CALL hid_get_manufacturer_string(hid_device* dev, wchar_t* string,
                                                                 size_t maxlen) {
  if (!string || !maxlen) {
    register_string_error(dev, L"Zero buffer/length");
    return -1;
...
int HID_API_EXPORT_CALL HID_API_CALL hid_get_product_string(hid_device* dev, wchar_t* string,
                                                            size_t maxlen) {
  if (!string || !maxlen) {
    register_string_error(dev, L"Zero buffer/length");
    return -1;
...
int HID_API_EXPORT_CALL HID_API_CALL hid_get_serial_number_string(hid_device* dev, wchar_t* string,
                                                                  size_t maxlen) {
  if (!string || !maxlen) {
    register_string_error(dev, L"Zero buffer/length");
    return -1;
...
int HID_API_EXPORT_CALL HID_API_CALL hid_get_indexed_string(hid_device* dev, int string_index,
                                                            wchar_t* string, size_t maxlen) {
  BOOL res;

  res = HidD_GetIndexedString(dev->device_handle, string_index, string,
                              sizeof(wchar_t) * (DWORD)MIN(maxlen, MAX_STRING_WCHARS));
  if (!res) {
...
int HID_API_EXPORT_CALL hid_winapi_get_container_id(hid_device* dev, GUID* container_id) {
  ...
  cr = CM_Get_DevNode_PropertyW(dev_node, &DEVPKEY_Device_ContainerId, &property_type,
                                (PBYTE)container_id, &len, 0);
...
int HID_API_EXPORT_CALL hid_get_report_descriptor(hid_device* dev, unsigned char* buf,
                                                  size_t buf_size) {
  PHIDP_PREPARSED_DATA pp_data = NULL;
